...
public class StudentDaoImpl implements StudentOrderDao {
    ...
    private static final String SELECT_ORDERS =
            "SELECT so.*, ro.register_office_area_id, ro.register_office_name, " +
                    "po_h.office_area_id as h_p_office_area_id, " +
                    "po_h.passport_office_name as h_p_office_name, " +
                    "po_w.office_area_id as w_p_office_area_id, " +
                    "po_w.passport_office_name as w_p_office_name " +
                    "FROM jc_student_order so " +
                    "INNER JOIN jc_register_office ro ON ro.register_office_id = so.register_office_id " +
                    "INNER JOIN jc_passport_office po_h ON po_h.passport_office_id = so.h_passport_office_id " +
                    "INNER JOIN jc_passport_office po_w ON po_w.passport_office_id = so.w_passport_office_id " +
    ...
    private static final String INSERT_CHILD =
            "INSERT INTO jc_student_child(" +
                    "student_order_id, child_sur_name, child_given_name," +
                    " child_patronymic, child_date_of_birth, child_certificate_number, child_certificate_date, " +
                    "child_register_office_id, child_post_index, child_street_code," +
                    " child_building, child_extension, child_apartment)" +
                    "VALUES (?, ?, ?," +
                    " ?, ?, ?, ?, " +
                    " ?, ?, ?, " +
                    " ?, ?, ?);";
    ...
    private static final String SELECT_CHILD =
            "SELECT soc.*, ro.register_office_area_id, ro.register_office_name " +
                    ...
                    "INNER JOIN jc_register_office ro ON ro. register_office_id = soc.child_register_office_id " +
    ...
    private void setParamsForAdult(PreparedStatement stmt, int start, Adult adult) throws SQLException {
        ...
        stmt.setString(start + 1, adult.getGivName());
        stmt.setString(start + 2, adult.getPatronmyc());
    ...
    private void setParamsForPerson(PreparedStatement stmt, int start, Person person) throws SQLException {
        ...
        stmt.setString(start + 1, person.getGivName());
        stmt.setString(start + 2, person.getPatronmyc());
...
//        return getStudentOrderTwoSelect();
    ...
    private Adult fillAdult(ResultSet rs, String pref) throws SQLException {
        ...
        adult.setGivName(rs.getString(pref + "given_name"));
    ...
    private Child fillChild(ResultSet rs) throws SQLException {
        String surName = rs.getString("child_sur_name");
        String givName = rs.getString("child_given_name");
        String patronymic = rs.getString("child_patronymic");
        LocalDate dateOfBirth = rs.getDate("child_date_of_birth").toLocalDate();

        Child child = new Child(surName, givName, patronymic, dateOfBirth);
        ...
        Long roId = rs.getLong("child_register_office_id");
        String roArea = rs.getString("register_office_area_id");
        String roName = rs.getString("register_office_name");
        RegisterOffice ro = new RegisterOffice(roId, roArea, roName);
        child.setIssueDepartment(ro);

        Address adr = new Address();
        Street st = new Street(rs.getLong("child_street_code"), "");
        adr.setStreet(st);
        adr.setPostCode(rs.getString("child_post_index"));
        adr.setBuilding(rs.getString("child_building"));
        adr.setExtension(rs.getString("child_extension"));
        adr.setApartment(rs.getString("child_apartment"));
